...
function getNearestCityToPosition(position, countryFilter = null) {
    let nearest = null;
    let minDist = Infinity;
    for (const [id, city] of Object.entries(cityManager)) {
        if (countryFilter && city.country !== countryFilter) continue;
        const n = nation[city.country];
        if (!n?._position) continue;
        const d = position.distanceTo(n._position);
        if (d < minDist) { minDist = d; nearest = { id, city, distance: d }; }
    }
    return nearest;
}

function captureNearestCity(attacker, defeatedUnit) {
    const target = getNearestCityToPosition(defeatedUnit.object.position, defeatedUnit.country);
    if (!target || target.distance > 0.9) return false;
    const previousOwner = target.city.country;
    target.city.country = attacker.country;
    target.city.fortification = Math.max(0, target.city.fortification - 15);
    target.city.supply = Math.max(20, target.city.supply - 25);
    if (target.city.garrison === defeatedUnit.id) target.city.garrison = null;
    addBattleLog(`🏳️ ${target.city.name} captured by ${nation[attacker.country]?.name || attacker.country} from ${nation[previousOwner]?.name || previousOwner}`);
    addNotification(`🏳️ ${target.city.name} captured!`, NOTIFICATION_TYPES.SUCCESS);
    toast(`🏳️ ${target.city.name} captured`);
    return true;
}
...
function executeAttack(attacker, defender) {
    ...
    if (attacker.country === defender.country) { toast('Cannot attack allied forces'); return; }

    const targetCity = getNearestCityToPosition(defender.object.position, defender.country);
    const terrain = targetCity && targetCity.distance < 0.55 ? 'CITY' : 'PLAIN';
    const result = executeBattle(attacker, defender, terrain, weather);
    if (!result) return;
    ...
    const report = generateBattleReport(attacker, defender, result, terrain, weather);
    addBattleLog(report.replace(/\n/g, ' • '));
    toast(`${attacker.name} dealt ${result.damage} damage to ${defender.name}`);

    if (result.destroyed) {
        destroyUnit(defender, attacker);
        captureNearestCity(attacker, defender);
    } else if (result.defenderOrg < 25) {
        defender.state = 'RETREATING';
        const retreat = defender.object.position.clone().multiplyScalar(1.04);
        defender.destination = retreat;
        toast(`↩️ ${defender.name} is retreating`);
    }
...
function getCountryOverview(countryId) {
    ...
    const avgStability = cities.length ? cities.reduce((sum, [, c]) => sum + (c.happiness || 0), 0) / cities.length : 0;
    const byType = {};
    army.forEach(u => byType[u.type] = (byType[u.type] || 0) + 1);
    return { cities, army, totalPop, industry, avgStability, byType };
}

function setCommandCountry(countryId) {
    if (!nation[countryId]) return;
    currentCountry = countryId;
    toast(`🇺🇳 Command country: ${nation[countryId].name}`);
    addNotification(`🇺🇳 Now commanding ${nation[countryId].name}`, NOTIFICATION_TYPES.INFO);
    updateAllUI();
...
function manageCountry(countryId) {
    ...
    const isPlayer = currentCountry === countryId;
    const armyRows = Object.entries(o.byType).map(([type, count]) => `<span class="city-tag">${UNITS_DATA[type]?.icon || '🪖'} ${type}: ${count}</span>`).join('') || '<span style="color:var(--muted);font-size:10px;">No active forces</span>';
    content.innerHTML = `
        <div class="info-card">
            ...
            <div class="stat-row"><span>Controlled Cities</span><b>${o.cities.length}</b></div>
            <div class="stat-row"><span>Army Strength</span><b>${o.army.length} units</b></div>
            <div class="stat-row"><span>Avg. City Morale</span><b>${Math.round(o.avgStability)}%</b></div>
            <div style="margin-top:8px;display:flex;flex-wrap:wrap;gap:4px;">${armyRows}</div>
            <button class="action-btn success" onclick="window.setCommandCountry('${countryId}')">🎖️ ${isPlayer ? 'Currently Commanding' : 'Take Command'}</button>
        </div>
        <div class="info-card">
            <h3>🏙️ Cities & Defense</h3>
            ${o.cities.map(([id, city]) => `<div class="stat-row">
                <span>${city.name}${city.fortification >= 50 ? ' 🛡️' : ''}</span>
                <span style="color:var(--muted);font-size:9px;">Pop ${formatNumber(city.population)} • Fort ${city.fortification}%</span>
                <button class="action-btn info" style="padding:2px 8px;font-size:8px;width:auto;margin:0;" onclick="window.showCityInfo('${id}')">View</button>
            </div>`).join('')}
        </div>
        <div class="info-card">
            <h3>🪖 Army Operations</h3>
            ${isPlayer ? `
                ...
                <button class="action-btn" onclick="window.trainUnitFromCity('${data.states[0].toUpperCase()}','TANK')">🔩 Train Tank</button>
                <button class="action-btn" onclick="window.trainUnitFromCity('${data.states[0].toUpperCase()}','ARTILLERY')">💥 Train Artillery</button>
                <button class="action-btn" onclick="window.trainUnitFromCity('${data.states[0].toUpperCase()}','AIR')">✈️ Train Aircraft</button>
            ` : '<p style="color:var(--muted);font-size:10px;">Take command of this country to train its forces.</p>'}
        </div>
    `;
}
