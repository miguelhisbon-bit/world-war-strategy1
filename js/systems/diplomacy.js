...
export function formAlliance(country1, country2) {
    ...
    alliances[key1] = { countries: [country1, country2], formed: Date.now(), mutualDefense: true };
...
export function breakAlliance(country1, country2) {
    const key1 = `${country1}_${country2}`;
    const key2 = `${country2}_${country1}`;
    if (alliances[key1]) { delete alliances[key1]; diplomacy[country1] = Math.max(-50, (diplomacy[country1] || 0) - 20); diplomacy[country2] = Math.max(-50, (diplomacy[country2] || 0) - 20); return true; }
    if (alliances[key2]) { delete alliances[key2]; diplomacy[country1] = Math.max(-50, (diplomacy[country1] || 0) - 20); diplomacy[country2] = Math.max(-50, (diplomacy[country2] || 0) - 20); return true; }
    return false;
}
...
export function declareWar(declarer, target) {
    ...
    const existingWar = wars.find(w => (w.country1 === declarer && w.country2 === target) || (w.country1 === target && w.country2 === declarer));
    if (existingWar) return false;
    wars.push({ country1: declarer, country2: target, started: Date.now(), score1: 0, score2: 0, casualties1: 0, casualties2: 0 });
