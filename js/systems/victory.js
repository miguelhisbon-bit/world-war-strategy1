...
export const victoryConditions = {
    military: {
        ...
        description: 'Control 60% of all cities',
        achieved: false
    },
    economic: {
        name: 'Economic Victory',
        description: 'Reach $1,000,000 GDP',
        achieved: false
    },
    diplomatic: {
        name: 'Diplomatic Victory',
        description: 'Form alliances with 80% of countries',
        achieved: false
    },
    research: {
        name: 'Research Victory',
...
export function checkVictoryConditions(units, nation, diplomacy, wars) {
    ...
    for (const [key, value] of Object.entries(results)) {
        if (value.achieved) {
            victory = {
                type: key,
                title: `${key.charAt(0).toUpperCase() + key.slice(1)} Victory!`,
                message: victoryConditions[key]?.description || 'You have achieved victory!',
                icon: getVictoryIcon(key)
            };
            break;
        }
    }
...
export function getVictoryStatus() {
    return {
        ...
        summary: Object.entries(victoryConditions).map(([key, value]) => ({
            type: key,
            name: value.name,
