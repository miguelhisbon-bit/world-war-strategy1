...
export function calculateWarScore(warId, units) {
    ...
    return {
        ...
        diff: Math.round(score1 - score2),
        casualties1: casualties1,
        casualties2: casualties2,
        battlesWon1: battlesWon1,
        battlesWon2: battlesWon2
    };
}
...
export function updateWarScore(warId, units) {
    ...
    if (scores) {
        war.score1 = scores.score1;
        war.score2 = scores.score2;
    }
}
...
export function getWarStatus(warId) {
    ...
    if (scores.diff > 30) {
        status = 'DECISIVE_ADVANTAGE';
        advantage = war.country1;
    } else if (scores.diff > 10) {
        status = 'ADVANTAGE';
        advantage = war.country1;
    } else if (scores.diff < -30) {
        status = 'DECISIVE_ADVANTAGE';
        advantage = war.country2;
    } else if (scores.diff < -10) {
        status = 'ADVANTAGE';
        advantage = war.country2;
    }
