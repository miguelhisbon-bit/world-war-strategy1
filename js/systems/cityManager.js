...
export function getCity(cityId) {
    return cityManager[cityId] || null;
}
...
export function getCityProduction(cityId) {
    const city = cityManager[cityId];
    if (!city) return null;
    return city.production;
}

export function getCityGarrison(cityId) {
    const city = cityManager[cityId];
    if (!city) return null;
    return city.garrison;
}

export function setCityGarrison(cityId, unitId) {
    const city = cityManager[cityId];
    if (!city) return false;
    city.garrison = unitId;
    return true;
}

export function improveCityFortification(cityId, amount = 5) {
    const city = cityManager[cityId];
    if (!city) return false;
    city.fortification = Math.min(100, city.fortification + amount);
    return true;
}
...
export function getCityStats(cityId) {
    ...
    return {
        ...
        fortification: city.fortification,
        supply: city.supply,
        happiness: city.happiness,
        unemployment: city.unemployment,
        buildings: city.buildings.length,
        hasGarrison: !!city.garrison
    };
}

export function getAllCities() {
    return Object.entries(cityManager).map(([id, data]) => ({
        id,
        ...data
    }));
}
