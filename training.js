...
export function trainUnit(cityId, unitType, country = 'BANGLADESH') {
    ...
    trainingQueue.push({ id: `train_${Date.now()}_${Math.random()}`, cityId, unitType, country, progress: 0, totalTime: unitData.buildTime || 20, status: 'QUEUED' });
    return true;
}

export function getTrainingQueue() { return trainingQueue; }
...
export function processTraining(dt, units, nation) {
    for (let i = trainingQueue.length - 1; i >= 0; i--) {
        ...
        if (item.progress >= item.totalTime) {
            ...
            if (unit) trainingQueue.splice(i, 1);
        }
    }
}
...
function spawnTrainedUnit(item, units, nation) {
    ...
    const unit = window.create3DUnit ? window.create3DUnit(unitName, item.unitType, new THREE.Vector3(center.x + offsetX, 0, center.z + offsetZ), true, item.country) : null;
    if (unit) { unit.city = item.cityId; if (units) units.push(unit); return unit; }
    return null;
}
